...
def merge_reviews_metadata(products_df, metadata_df):
    metadata_cols = ['asin', 'title', 'description', 'price', 'brand']
    merged_df = pd.merge(products_df, metadata_df[metadata_cols], left_on='asin', right_on='asin', how='inner')
    return merged_df


def main_3(): #  ~ 1 hour run time
    print("Opening products.csv")
    products_df = pd.read_csv('E:/Amazon_Review_Dataset/stage_2_dfs_nk/products.csv')
    chunksize = 100000
    ...
    for metadata_chunk in pd.read_json('E:/Amazon_Review_Dataset/All_Amazon_Meta.json/All_Amazon_Meta.json', lines=True, chunksize=chunksize):
    ...
    merged_df.to_csv('E:/Amazon_Review_Dataset/stage_3_dfs_nk/products.csv', index=False)
