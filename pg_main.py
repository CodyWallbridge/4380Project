...
def populateTables():
    ...
    data_types = {
        'brand_id': sqlalchemy.Integer,
        'brand_name': sqlalchemy.String
    }
    index_col = 'brand_id'
    dfBrands = pd.read_csv("complete_data_36gb/brands.csv")
    dfBrands = dfBrands.reset_index(drop=True)
    dfBrands.set_index('brand_id', inplace=True)
    dfBrands.to_sql('brands', con=con, if_exists='replace', index=True, index_label=index_col, dtype=data_types, chunksize=50000, method='multi')

    print("handling categories")
    data_types = {
        'category_id': sqlalchemy.Integer,
    ...
    dfCategories.set_index('category_id', inplace=True)
    dfCategories.to_sql('categories', con=con, if_exists='replace', index=True, index_label=index_col, dtype=data_types, chunksize=50000, method='multi')

    print("handling customers")
    data_types = {
        'reviewerID': sqlalchemy.String,
        'reviewerName': sqlalchemy.String
    }
    index_col = 'reviewerID'
    ...
    dfCustomers.set_index('reviewerID', inplace=True)
    dfCustomers.to_sql('customers', con=con, if_exists='replace', index=True, index_label=index_col, dtype=data_types, chunksize=50000, method='multi')
    ...
    dfProducts.set_index('asin', inplace=True)
    dfProducts.to_sql('products', con=con, if_exists='replace', index=True, index_label=index_col, dtype=data_types, chunksize=100000, method='multi')

    '''
    data_types = {
        'asin': sqlalchemy.String,
        'reviewerID': sqlalchemy.String,
        'unixReviewTime': sqlalchemy.Integer,
        'reviewText': sqlalchemy.String,
        'summary': sqlalchemy.String,
        'overall': sqlalchemy.Integer,
        'vote': sqlalchemy.Integer,
    }
    index_col = ['asin', 'reviewerID']
    # for i in range(0,1):
    ...
    for i in range(0,32):
        ...
        dfReviews.set_index(['asin', 'reviewerID'], inplace=True)

        if i == 0:
            dfReviews.to_sql('reviews', con=con, if_exists='replace', index=True, index_label=index_col, dtype=data_types, chunksize=50000, method='multi')
        else:
            dfReviews.to_sql('reviews', con=con, if_exists='append', index=True, index_label=index_col, dtype=data_types, chunksize=50000, method='multi')
        con.commit()
